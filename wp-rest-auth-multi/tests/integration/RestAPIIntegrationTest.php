...
class RestAPIIntegrationTest extends WP_UnitTestCase
{
    ...
    public function test_jwt_routes_are_registered(): void
    {
        ...
        $jwt_routes_exist = array_key_exists('/wp-rest-auth-multi/v1/jwt/token', $routes) ||
                           array_key_exists('/wp-rest-auth-multi/v1/jwt/refresh', $routes);
        ...
        if (!$jwt_routes_exist) {
            ...
            $this->assertArrayHasKey('/wp-rest-auth-multi/v1/jwt/token', $routes);
            $this->assertArrayHasKey('/wp-rest-auth-multi/v1/jwt/refresh', $routes);
        }
    }
    ...
    public function test_oauth2_routes_are_registered(): void
    {
        ...
        // Check if routes exist (they should be registered by the plugin)
        $oauth2_routes_exist = array_key_exists('/wp-rest-auth-multi/v1/oauth2/authorize', $routes) ||
                              array_key_exists('/wp-rest-auth-multi/v1/oauth2/token', $routes);
        ...
        if (!$oauth2_routes_exist) {
            ...
            $this->assertArrayHasKey('/wp-rest-auth-multi/v1/oauth2/authorize', $routes);
            $this->assertArrayHasKey('/wp-rest-auth-multi/v1/oauth2/token', $routes);
        }
    }

    public function test_jwt_token_issuance_with_valid_credentials(): void
    {
        $request = new WP_REST_Request('POST', '/wp-rest-auth-multi/v1/jwt/token');
